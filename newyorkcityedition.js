// New York City edition. Square/Card/utiltext live in edition-common.js.
...
// City tax popup with the "pay flat $200 or 10% of net worth" option.
// Wires button clicks via real handlers (not inline strings) so the popup
// can use i18n keys and survive player names containing quotes.
function citytax() {
	var p = player[turn];

	if (p.human) {
		var HTML = t('popup.cityTaxChoice')
			+ "<div>"
			+ "<input type='button' id='citytax-pay200' value='" + t('ui.pay200') + "' />"
			+ "<input type='button' id='citytax-paypercent' value='" + t('ui.payPercent') + "' />"
			+ "</div>";

		popup(HTML, null, "blank");

		var pay200Btn = document.getElementById('citytax-pay200');
		...
		if (pay200Btn) {
			pay200Btn.addEventListener('click', function () {
				closePopup();
				var pNow = player[turn];
				addAlert(t('alert.paidCityTax', { player: pNow.name }));
				pNow.pay(200, 0);
			});
		}
		if (payPercentBtn) {
			payPercentBtn.addEventListener('click', function () {
				closePopup();
				var pNow = player[turn];
				var cost = pNow.money;
				for (var i = 0; i < 40; i++) {
				...
				addAlert(t('alert.paidCityTax', { player: pNow.name }) + ' ($' + cost + ')');
				pNow.pay(cost, 0);
			});
		}
	} else {
		addAlert(t('alert.paidCityTax', { player: p.name }));
		p.pay(200, 0);
	}
}
...
square[26] = new Square("Deloitte & Touche LLP", "$260", "#E6C200", 260, 8, 22, 110, 330, 800, 975, 1150);
square[27] = new Square("SmithBarney", "$260", "#E6C200", 260, 8, 22, 110, 330, 800, 975, 1150);
square[28] = new Square("Con Edison Gas", "$150", "white", 150, 2);
square[29] = new Square("CITIBANK", "$280", "#E6C200", 280, 8, 24, 120, 360, 850, 1025, 1200);
...
chanceCards[0] = new Card("Get out of Jail free. This card may be kept until needed or sold.", function(p) { p.chanceJailCard=true; updateOwned();});
